...
from binerised import CD_bin_95
...
def fast(i, df):
    ...
    edges = pd.DataFrame(data=edge_data, columns=['source', 
                                                  'target',
                                                  'String_Score'])
    edges.to_csv("patient_networks/CD_95_network_3/"+str(df.index[i])+".txt",
                 sep="\t",
                 header=False,
                 index=False,
                 encoding="utf-8")
...
def fast2(i, df):
    ...
    edges = pd.DataFrame(data=edge_data, columns=['source', 
                                                  'target',
                                                  'String_Score'])
    edges.to_csv("patient_networks/CD_95_network_physical/"+str(df.index[i])+".txt",
                 sep="\t",
                 header=False,
                 index=False,
                 encoding="utf-8")
