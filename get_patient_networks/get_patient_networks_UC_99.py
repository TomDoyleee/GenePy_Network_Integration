...
from binerised import UC_bin_97_5, CD_bin_97_5, CD_bin_99, UC_bin_99, CD_bin_95, UC_bin_95
...
def fast(i, df):
    ...
    edges = pd.DataFrame(data=edge_data, columns=['source', 
                                                  'target',
                                                  'String_Score'])
    edges.to_csv("patient_networks/UC_99_network_3/"+str(df.index[i])+".txt",
                 sep="\t",
                 header=False,
                 index=False,
                 encoding="utf-8")
...
def fast2(i, df):
    ...
    edges = pd.DataFrame(data=edge_data, columns=['source', 
                                                  'target',
                                                  'String_Score'])
    edges.to_csv("patient_networks/UC_99_network_physical/"+str(df.index[i])+".txt",
                 sep="\t",
                 header=False,
                 index=False,
                 encoding="utf-8")
